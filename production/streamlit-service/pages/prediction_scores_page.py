...
def main():
    ...
    integer_grades_flag = st.checkbox("There can only be integer estimates?", value=True)
    ...
    if st.button("✅ Submit"):
        ...
        params =  {'header' : None if not table_heading  else 0, 'index_col' : False if not table_index else 0}
        ...
        try:
            ...
            if not table_heading:
                df.columns = ["lesson_"+i for i in range(1, df.shape[1]+1)]
        ...
        try:
            request_data = {"data" : df.to_dict(orient="records"), "min_value": start_val, "max_value": end_val, "integer_grades_flag": integer_grades_flag}
            response = requests.post(f"http://{ip_api}:{port_api}/predict_carte_model", json=request_data)
