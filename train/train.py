...
def load_and_split_dataset(dataset_path: str, tokenizer, max_seq_length = 4096, train_proportion = 0.85):
    ...
    #dataset = dataset.select(list(range(22)))
    ...
    prompt_lens = pd.Series([len(tokenizer.encode(x)) for x in dataset['text']])
    dataset = dataset.select(prompt_lens.index[prompt_lens<max_seq_length])
    split_dataset = dataset.train_test_split(train_size=train_proportion, seed=42)
...
def download_checkpoint(s3_client, prefix='', bucket_name = '',local_dir = 'data/model/lora_adapters'):
    ...
    if checkpoints:
        ...
        response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=checkpoint_folder)
...
def main():
    ...
    task_name = f'fine-tuning-qwen14b, LoRA'
    ...
    train_dataset, val_dataset = load_and_split_dataset("/data/finetuning-data/", tokenizer, max_seq_length = max_seq_length)
    ...
    s3_callback = S3UploadCallback(
        s3_client=s3_client,
        bucket_name='',
        s3_prefix='qwen14b-weights/'+s3_folder
    )

    resume_checkpoint = None
    local_adapter_path = '/data/lora-checkpoint'
    try:
        download_checkpoint(s3_client, prefix='qwen14b-weights/'+s3_folder+'/', local_dir=local_adapter_path)
    ...
    trainer = SFTTrainer(
        ...
        args = SFTConfig(
            ...
            gradient_accumulation_steps = gradient_accumulation_steps, # Fixed major bug in latest Unsloth
            #warmup_steps = 25,
            warmup_ratio = 0.1,
